...
/**
 * Builds the memory consolidation prompt for auto-dream.
 * Java equivalent of Python services/autodream/prompt.py.
 */
public final class AutoDreamPrompt {

    private static final int MAX_ENTRYPOINT_LINES = 200;
    private static final String ENTRYPOINT_NAME = "MEMORY.md";

    private AutoDreamPrompt() {}

    /**
     * Python build_consolidation_prompt: builds the 4-phase dream prompt.
     */
    public static String buildConsolidationPrompt(
            Path memoryRoot, Path sessionDir, String extra, boolean preview) {
        String memoryPath = memoryRoot.toAbsolutePath().normalize().toString();
        String sessionPath = sessionDir.toAbsolutePath().normalize().toString();
        String extraSection = extra != null && !extra.strip().isEmpty()
                ? "\n\n## Additional context\n\n" + extra.strip() : "";
        String writeMode = preview
        ...
        return """
                ...
                Current date: %s
                Memory directory: `%s`
                Session snapshots: `%s` (JSON files can be large; inspect narrowly, do not dump everything)
                Mode: %s
                ...
                - Read `%s` if present; it is the memory index.
                ...
                Update `%s` so it stays under %d lines and remains an index, not a content dump.
                ...
                If nothing changed, say so and explain why.%s"""
                .formatted(
